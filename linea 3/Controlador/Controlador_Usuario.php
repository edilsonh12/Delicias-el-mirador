<?php
    ...
    if(isset($_POST['action'])  && $_POST['action']=='iniciarSesion'){
        ...
        if($estado==true){
            ...
            fwrite($logFile, "\n".date("d/m/Y H:i:s")."   ".$control." "."usuario: "." ".$usuario) or die("Error escribiendo en el archivo");
            fclose($logFile);
 ...
 if(isset($_POST['action'])  && $_POST['action']=='SalirRegistroUsuario'){
    ...
    if(isset($_POST['action'])  && $_POST['action']=='cerrarSesion'){
        ...
        session_unset();
        session_destroy();
        echo "<script> alert('Gracias por preferinos, hasta la próxima.'); window.location= '../Vista/index.html' </script>";
